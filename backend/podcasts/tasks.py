...
def add_episode(feed_url):
    ...
    """
    logger.info(f"Starting add_episode task for feed: {feed_url}")
    updater = EpisodeUpdater([feed_url])
    updater.populate()
    logger.info(f"Finished add_episode task for feed: {feed_url}")
...
def remove_podcasts():
    ...
    deleted_count, _ = Podcast.objects.filter(episodes__isnull=True).delete() # This might be wrong if episodes is a reverse relation that can be empty but not null in join.
    # Better approach matching legacy logic:
    podcasts = Podcast.objects.annotate(num_episodes=Count('episodes')).filter(num_episodes=0)
