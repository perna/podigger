    ...
    def handle(self, *args, **options):
        ...
        for model_label in preferred_order:
            ...
            try:
                ...
                self.stderr.write(self.style.WARNING(f"Unknown model in fixture: {model_label}, skipping."))
                continue
        ...
        self.stdout.write(f"Found {total} records across {len(to_delete)} models to delete.")
        ...
        with transaction.atomic():
            for model_label, count, qs in to_delete:
                if count:
                    qs.delete()
                    self.stdout.write(self.style.SUCCESS(f"Deleted {count} records from {model_label}"))
