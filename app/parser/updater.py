...
class EpisodeUpdater(object):
    ...
    def populate(self):
            #for link in self.feeds:
                #print(link)
        try:
            pod = Podcast.query.filter_by(feed=self.feeds).first()
            print(pod.name)
 
            podcast = get_episodes(self.feeds)
            pod.image = podcast['image']
            #pod.total_episodes = len(podcast['items']
            ...
            for item in podcast['items']:
                ...
                try:
                    ...
                    print("adicionar episodio ", ep.title)
                    db.session.add(ep)
                except IntegrityError as error:
                    print("não foi possível adicionar o episodio", error)
                    pass
 
            db.session.commit()
            print("episódio adicionado")
        except:
            db.session.close()
