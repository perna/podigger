...
class Podcast(Base):
    ...
    image = db.Column(db.String(), nullable=True, default='/static/dist/img/podcast-banner.png')
    language_id = db.Column(db.Integer, db.ForeignKey('podcast_language.id'), default=1)
...
class PodcastLanguage(Base):
    ...
    code = db.Column(db.Text(), nullable=True, default='pt')
...
class Episode(Base):
    ...
    tags = db.relationship('Tag', secondary=tags, backref=db.backref('episodes', lazy=True))
